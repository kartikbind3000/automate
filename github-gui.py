#pip install pyinstaller pyinstaller --onefile --windowed --name "GitHubToolPro" --icon=github.ico github_tool_pro.py
import os
import json
import base64
import threading
...
║                                                                          ║
║   ⚡ New Features: Dashboard, Smart Sync, Batch Delete                   ║
║                                                                          ║
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
class GitHubGodMode(ctk.CTk):
    def __init__(self):
        ...
        self.title("GitHub Tool Pro - Krishna Edition")
    ...
    def _build_sidebar(self):
        ...
        logo = ctk.CTkLabel(self.sidebar, text="GITHUB-GUI", font=ctk.CTkFont(size=22, weight="bold"))
        logo.pack(pady=30, padx=20)

        self._sidebar_btn("🏠 Dashboard", "home")
        self._sidebar_btn("📁 Browse Repos", "list")
        self._sidebar_btn("📤 Upload New", "upload")
        self._sidebar_btn("📥 Clone Repo", "clone")
        self._sidebar_btn("⚙️ Settings", "settings")

        self.status_lbl = ctk.CTkLabel(self.sidebar, text="Disconnected", text_color="gray")
    ...
    def _build_main_content(self):
        ...
        f_home = ctk.CTkFrame(self.container, fg_color="transparent")
        banner_label = ctk.CTkLabel(f_home, text=BANNER, font=("Consolas", 12), justify="left", text_color=GH_BLUE)
        banner_label.pack(pady=20, padx=10)
        ctk.CTkLabel(f_home, text="Welcome to the Ultimate GitHub Manager", font=("Arial", 18, "italic")).pack()
        self.frames["home"] = f_home
        ...
        self.search_entry = ctk.CTkEntry(search_row, placeholder_text="Search your repositories...", width=400)
        self.search_entry.pack(side="left", padx=5)
        self.search_entry.bind("<KeyRelease>", self.filter_repos)
        ctk.CTkButton(search_row, text="Refresh List", fg_color=GH_BLUE, width=100, command=self.fetch_repos).pack(side="right")
        ...
        self.progress = ctk.CTkProgressBar(f_up, width=500, progress_color=GH_GREEN)
        self.progress.set(0)
        ...
        ctk.CTkButton(f_cl, text="Browse Destination", fg_color=GH_BORDER, command=lambda: self.browse_folder(self.cl_dest)).pack(pady=5)
    ...
    def _build_log_area(self):
        ...
        self.log("Ready to execute commands...")
        ...
        def task():
            try:
                ...
                if r.status_code == 200:
                    ...
                    self.after(0, lambda: self.status_lbl.configure(text=f"Auth: {self.username}", text_color=GH_BLUE))
                    self.fetch_repos()
                else:
    ...
    def render_repo_list(self, repos):
        ...
        for repo in repos:
            frame = ctk.CTkFrame(self.repo_scroll, fg_color=GH_BORDER, height=55)
            ...
            ctk.CTkButton(frame, text="Delete", fg_color=GH_RED, width=80, height=30, 
                          command=lambda n=repo['name']: self.delete_repo(n)).pack(side="right", padx=10)
            
            ctk.CTkButton(frame, text="Update/Sync", fg_color=GH_GREEN, width=100, height=30, 
                          command=lambda n=repo['name']: self.sync_existing_repo(n)).pack(side="right", padx=5)

    def delete_repo(self, name):
        if messagebox.askyesno("CONFIRM DELETION", f"WARNING: This action is permanent!\n\nAre you sure you want to delete repository: '{name}'?"):
            headers = {'Authorization': f'token {self.token}'}
            def task():
                r = requests.delete(f'https://api.github.com/repos/{self.username}/{name}', headers=headers)
                if r.status_code == 204:
                    self.log(f"✓ Successfully Deleted: {name}")
                    self.fetch_repos()
                else:
                    self.log(f"✗ Failed to delete {name}")
            threading.Thread(target=task, daemon=True).start()
    ...
    def sync_existing_repo(self, repo_name):
        ...
        if messagebox.askyesno("Confirm Sync", f"Selected Path: {local_path}\nTarget Repo: {repo_name}\n\nDo you want to start the synchronization?"):
            self.show_frame("upload")
            self.up_name.delete(0, "end")
            self.up_name.insert(0, repo_name)
            self.up_path.delete(0, "end")
            self.up_path.insert(0, local_path)
    ...
    def start_new_upload(self):
        ...
        if not repo_name or not local_path: 
            messagebox.showwarning("Input Error", "Please provide repository name and local path.")
            return
        
        if messagebox.askyesno("Confirm Upload", f"Create/Update repo: {repo_name}?\nLocal Source: {local_path}"):
    ...
    def engine_upload(self, repo_name, local_dir, create_new=False, private=False):
        ...
        if create_new:
            self.log(f"Creating/Checking repository {repo_name}...")
            requests.post('https://api.github.com/user/repos', headers=headers, json={'name': repo_name, 'private': private, 'auto_init': True})

        # Fetch Tree SHAs for smart skipping
        self.log("Fetching remote tree status...")
        existing_shas = {}
        ...
        ignore_list = ['.git', '__pycache__', 'node_modules', '.DS_Store', '.env']
        files_to_process = [f for f in root.rglob('*') if f.is_file() and not any(x in f.parts for x in ignore_list)]

        total = len(files_to_process)
        if total == 0:
            self.log("No files found to upload.")
            return

        self.log(f"Processing {total} files using 10-worker engine...")
        ...
        def process_file(file_path):
            ...
            data = {"message": "Sync via Krishna GITHUB Tool", "content": encoded}
    ...
    def save_token(self):
        ...
        if t:
            ...
            self.token = t
            self.log("Token updated. Re-authenticating...")
            self.verify_auth()
